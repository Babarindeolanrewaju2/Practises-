...
def transform_logs(logs):
    ...
    daily_visitors = cleaned_logs.groupby(cleaned_logs["timestamp"].dt.date)["ip_address"].nunique()
...
def load_data(data):
    ...
    data.to_csv("/path/to/daily_visitors.csv", header=["date", "visitor_count"], index=True)
